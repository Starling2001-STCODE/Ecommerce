export default function Products({ products, openModal, cart, animatedProductId }) {
    const { id, name, image, description, price, stock } = products
    const productInCart = cart.find((item) => item.id === products.id);
            ...
            <div className="product-info p-2">
                ...
                <div className="info-row flex justify-between items-center">
                    ...
                    <div className="relative">
                        {/* Botón del carrito */}
                        <div className={`carrito ${animatedProductId === products.id ? "animate" : ""}`}>                            <button className="add-to-cart p-1 hover:bg-gray-200 rounded" onClick={() => openModal(products)}>
                            <svg
                            ...
                            {quantityInCart > 0 && (
                                <span className="absolute -top-2 -right-2 bg-[#CE1126] text-white text-xs rounded-full px-1.5 py-0.5">
                                    {quantityInCart}
                                </span>
                            )}
                        </div>
